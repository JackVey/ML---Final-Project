...
@st.cache_resource
def load_all_artifacts():
    # فرض بر این است که مسیرها درست هستند
    with open('saved_artifacts/pipeline_metadata.json', 'r') as f:
        metadata = json.load(f)
    scaler_fd001 = joblib.load('saved_artifacts/scaler_fd001.joblib')
    xgb_win_model = joblib.load('saved_artifacts/xgb_win_model.joblib')
    calibrated_models = joblib.load('saved_artifacts/calibrated_models.joblib')
    # models_unsupervised = joblib.load('saved_artifacts/models_unsupervised.joblib') # در صورت وجود
    return metadata, scaler_fd001, xgb_win_model, calibrated_models


metadata, scaler, xgb_model, calib_models = load_all_artifacts()

st.title("Turbine Predictive Maintenance Dashboard")
st.markdown("---")

st.subheader("Upload CMAPSS Sensor Data")
uploaded_file = st.file_uploader("Drop your test data file (.txt or .csv) here", type=["csv", "txt"])

if uploaded_file is not None:
    # حل مشکل خواندن فایل‌های تست با جداکننده اسپیس
    columns = ['engine_id', 'cycle', 'setting_1', 'setting_2', 'setting_3'] + [f's_{i}' for i in range(1, 22)]
    try:
        # تلاش برای خواندن به عنوان فایل اسپیس-سپریتد
    ...
    st.write("Preview of uploaded data (Last 5 cycles):")
    st.dataframe(df_uploaded.tail())

    if st.button("Run Diagnostics on Last Cycle", type="primary"):
        # استخراج آخرین ردیف برای پیش‌بینی
        latest_data = df_uploaded.iloc[[-1]]

        # در اینجا باید داده‌ها اسکیل شوند (با استفاده از scaler) و ویژگی‌های پنجره ساخته شود
        # برای سادگی فرض میکنیم داده آماده ورود به مدل است:
        # features = scaler.transform(latest_data[active_sensors])

        # -- شبیه‌سازی استخراج ویژگی جهت نمایش منطق --
        sample_input = np.random.randn(1, xgb_model.n_features_in_)  # موقت برای اجرای کد

        # ۱. پیش‌بینی RUL
        predicted_rul = float(xgb_model.predict(sample_input)[0])

        # ۲. ارزیابی ریسک خرابی در افق‌های زمانی
        # فرض می‌کنیم calib_models دیکشنری از مدل‌های کالیبره شده است
        risk_probs = {}
        if type(calib_models) == dict:
            for horizon in [10, 20, 30]:
                if f'rf_horizon_{horizon}' in calib_models:
                    prob = calib_models[f'rf_horizon_{horizon}'].predict_proba(sample_input)[0][1]
                    risk_probs[horizon] = prob

        # تولید مقادیر فرضی ریسک در صورت عدم وجود مدل برای جلوگیری از ارور داشبورد
        risk_30 = risk_probs.get(30, 0.85 if predicted_rul < 35 else 0.1)
        risk_10 = risk_probs.get(10, 0.95 if predicted_rul < 15 else 0.05)

        # ۳. شبیه‌سازی امتیاز ناهنجاری (در پروژه واقعی از مدل Unsupervised استفاده کنید)
        anomaly_score = 1.5  # مقدار نمایشی

        # === اعمال منطق ترکیب خروجی‌ها (گام ۸.۳) ===
        status = "CONTINUE"
        color = "🟢"
        trigger_msg = "All systems operating normally."

        if predicted_rul <= 15 or risk_10 > 0.7:
            status = "STOP"
            color = "🔴"
            trigger_msg = f"Critical! RUL is very low ({predicted_rul:.1f}) OR extreme short-term failure risk."
        elif predicted_rul <= 40 or risk_30 > 0.6 or anomaly_score > 2.0:
            status = "INSPECT"
            color = "🟡"
            trigger_msg = "Warning triggers activated: Elevated risk in 30-cycle horizon or detected anomalies."

        # === نمایش کارت‌های داشبورد ===
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("Estimated RUL", f"{predicted_rul:.1f} Cycles")
        with col2:
            st.metric("30-Cycle Risk", f"{risk_30 * 100:.1f}%")
        with col3:
            st.metric("Anomaly Score", f"{anomaly_score:.2f}")
        with col4:
            st.markdown(f"### Action: {color} {status}")
            st.caption(trigger_msg)